class UploadLayout extends HTMLElement {
	...
	render() {
		this.innerHTML = `
	   <main class="flex-grow  flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto w-full">
  <h2 class="text-center text-pink-400 text-xl sm:text-2xl font-bold mb-8 drop-shadow-[0_0_4px_rgba(255,182,193,0.7)]">
  ...
  <section class="bg-pink-400 rounded-2xl shadow-md px-4 sm:px-6 md:px-10 py-10 w-full relative">
    ...
    <p class="text-white text-center text-xs sm:text-sm leading-tight mb-10 max-w-xl mx-auto">
      Cek <span class="text-sky-300 font-semibold">Jerawat</span> kami siap membantumu menemukan obat jerawat yang paling cocok untuk kulitmu.
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10 max-w-full mx-auto text-white text-[10px] sm:text-xs leading-tight">
      ...
      <div class="flex flex-col items-center text-center px-2">
        <div class="bg-white text-pink-400 rounded-full w-12 h-12 mb-2 flex justify-center items-center">
    ...
    <button
      ...
      class="absolute left-1/2 -translate-x-1/2 mt-10 sm:mt-0 sm:-bottom-6 bg-sky-300 text-white font-semibold rounded-md px-6 py-2 text-sm shadow-lg hover:bg-sky-400 transition-all"
    >
      Unggah &amp; Prediksi
    </button>
  </section>
